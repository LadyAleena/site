...
B<Fancy::Sort::Split> returns the expression to split the values in lists for sort.
...
Fancy::Sort::Split returns the expression to split the values in lists for L<sort|https://perldoc.perl.org/functions/sort.html> subroutines using C<split_sort>. C<split_sort> has to be imported into your script.

C<split_sort> has four required parameters. The first and second paremeters are C<$a> and C<$b> from C<sort> or C<$b> and C<$a> if you want a descending sort. The third parameter is the expression you want to split the strings by. The fourth is the type of sort you want, C<number> or C<alpha> (C<letter>).

  split_sort($a, $b, 'type', 'expr');

A note of caution for the numerical sorts, when a number has a leading zero (C<02>), the leading zero will be dropped. So, C<02> will be the same as C<2>.
