...
my $select   = get_cgi_param($cgi, 'page');
...
my $text_directory = file_directory('Movies', 'text');
...
my $heading = length $series && $series_select->{$series}  ? textify_movie($series) :
              ...
              length $select && $select eq 'Movie opinions' ? 'Movie opinions' :
              q(Lady Aleena's movie lists);

page( 'heading' => $heading, 'selected' => $select, 'code' => sub {
  if ( $search || $alpha ) {
    section(3, sub {
    ...
    if ($genre_select->{$genre_list}) {
      ...
      my $genre_map = anchor('genre map', { href => '../Miscellany/index.pl?select=Genre+map' });
      section(3, sub {
